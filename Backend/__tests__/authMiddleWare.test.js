const protect = require("../src/middleware/authMiddleWare").default;
...
describe("auth middleware protect", () => {
	...
	test("should return 401 if user not found", async () => {
		...
		expect(res.json).toHaveBeenCalledWith({
			message: "Nicht autorisiert, Benutzer nicht gefunden",
		});
	});
	...
	test("should return 401 if no session", async () => {
		...
		expect(res.status).toHaveBeenCalledWith(401);
		expect(res.json).toHaveBeenCalledWith({
			message: "Nicht autorisiert, keine Session",
		});
	});
	...
	test("should return 500 on error", async () => {
		...
		expect(res.json).toHaveBeenCalledWith({
			message: "Serverfehler beim Authentifizieren",
		});
	});
});
