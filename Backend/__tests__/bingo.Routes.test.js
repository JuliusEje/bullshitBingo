...
const InformaticsLectureTerm = require("../src/models/InformaticsLecutreTerm").default;
const bingoRoutes = require("../src/routes/bingoRoutes").default; // ggf. Pfad anpassen
...
test("GET /api/bingo/meeting returns 25 phrases", async () => {
	...
	expect(MeetingBingoField.aggregate).toHaveBeenCalledWith([
		{ $sample: { size: 25 } },
	]);
});
...
test("GET /api/bingo/lecture returns 25 terms", async () => {
	...
	expect(InformaticsLectureTerm.aggregate).toHaveBeenCalledWith([
		{ $sample: { size: 25 } },
	]);
});
