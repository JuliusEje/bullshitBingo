...
const gameRoutes = require('../src/routes/gameRoutes'); // ggf. Pfad anpassen
...
test('GET /api/games/list should list games', async () => {
    Game.find.mockReturnValue({
        ...
        sort: jest.fn().mockResolvedValue([
            { _id: 'game123', players: [], winner: null }
        ])
    });
...
test('POST /api/games/bingo/:gameId should set winner and increment points', async () => {
    ...
    expect(response.body.success).toBe(true);
    expect(saveMock).toHaveBeenCalled();
    expect(User.findByIdAndUpdate).toHaveBeenCalledWith('user123', { $inc: { points: 100 } });
});
